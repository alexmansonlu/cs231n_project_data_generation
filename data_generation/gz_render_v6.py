...
from gazebo_msgs.srv import GetEntityState, SetEntityState, SpawnEntity, DeleteEntity
...
class DataRenderGazebo(Node):
    def __init__(self):
        ...
        # spawn and delete services
        while not self.delete_client.wait_for_service(timeout_sec=3.0):
    ...
    def data_loop(self):
        for i in range(SETUP_ITERATION):
            ...
            while True:
                ...
                cv2.imwrite('dataset/rgb/'+str(i)+'_'+str(self.sample_num)+'.png', self.rgb_image_checked)
                cv2.imwrite('dataset/depth/'+str(i)+'_'+str(self.sample_num)+'.png',cv_depthImage)
        ...
        self.get_logger().info('Dataset generation completed, feel free to close the terminal')

    # ========camera related=========
    def reset_cam(self):
        self.get_logger().info('Resetting the camera')
        self.phi = self.phi_init
        self.theta = self.theta_init
        self.dist = self.dist_init
    ...
    def delete_model(self, model_name,num):
        request = DeleteEntity.Request()
        request.name = model_name+str(num)
    ...
    def spawn_model(self, model_name, num):
        ...
        request.name = model_name+str(num)
        request.xml = self.load_model_xml(model_name)
        ...
        try:
            ...
            self.get_logger().error(f'Error in gazebo/spawn_model service request: {str(e)}')

    def load_model_xml(self, model_name):
    ...
    def generate_set(self):
        ...
        for i in range(num_model):
            self.models_param.append(MODEL_LIST[random.randint(0, len(MODEL_LIST)-1)])

        self.get_logger().info(f'Generated {num_model} models: {self.models_param}')
    # endregion
